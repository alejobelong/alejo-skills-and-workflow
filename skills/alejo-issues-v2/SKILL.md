...
description: Compact Alejo Issues v2 workflow that turns Linear PRD.xml, SAD.xml, Q&A.xml, CONTEXT.xml, prototype evidence, ADRs, review gaps, or existing Linear issues into self-contained production-ready feature Linear implementation issues with explicit providers, Doppler secret refs, XML run contracts, SAD-defined feature-folder code organization, and no mocks or placeholders.
---

# Alejo Issues v2

Create production-ready Linear implementation issues. Each issue is one full functional feature: one externally observable capability implemented end to end in the SAD-defined feature folder. Each issue must be executable by a fresh coding agent from the Linear body plus normal repo inspection.
...
5. Break work into features: one externally observable user/operator/tester capability per issue, aligned to one SAD-defined feature folder.
6. Publish accepted features to Linear in dependency order, using `ready-for-agent` only when all decisions and Doppler secret names are resolved.
...
- Keep feature-owned UI, routes/API, data access, provider adapters, tests, and contracts together in the SAD-defined feature folder. Use framework-required route/app files only as thin adapters.
...
Start with the smallest complete feature path that proves the capability end to end. Fold required schemas, migrations, validation, provider wiring, scheduled jobs, permissions, telemetry, and tests into the first feature that needs them.

Split large work into smaller complete features by user role, scenario, state, permission boundary, data scope, integration boundary, or happy path versus important edge case. Blockers should be prior observable behavior issues or unresolved human decisions, not vague technical phases.

## Verification Contract

Each issue body, or the generated BDD budget derived from it, must include:

- Representative prompts, inputs, payloads, or user actions for the feature; chat and managed-agent work must include realistic prompts and expected answer properties.
...
- Code organization points to one SAD-defined owning feature folder plus any thin framework adapters.

Show the proposed issue set before publishing with title, blockers, source refs, providers/secrets, observable outcome, surface, code organization, and why each is one complete feature.

## Issue XML

```xml
<?xml version="1.0" encoding="UTF-8"?>
<feature_issue type="AFK" readiness="{ready-for-agent|blocked}">
  ...
  <necessary_context>
    ...
    <domain_language><![CDATA[{CONTEXT.xml and Q&A.xml terms or decisions required for this feature.}]]></domain_language>
  ...
  <code_organization>
    <feature_folder>features/{feature-slug}/</feature_folder>
    <rule><![CDATA[Keep all code related to this feature in the SAD-defined feature folder: UI, API/route handling, data access, provider adapters, tests, and contracts.]]></rule>
    <framework_mapping><![CDATA[{Thin framework adapters required elsewhere, or None.}]]></framework_mapping>
    <shared_code><![CDATA[{Cross-cutting shared modules allowed for this feature and why, or None.}]]></shared_code>
  </code_organization>
  <run_contract>
    ...
    <quality_attributes><![CDATA[{Feature-relevant SAD.xml/SAT.xml quality targets.}]]></quality_attributes>
    <bdd_budget>{default 3 outer repair iterations unless SAD.xml/SAT.xml says otherwise}</bdd_budget>
    <representative_prompts><![CDATA[{Representative prompts, inputs, payloads, or user actions; required for chat/managed-agent features, otherwise None.}]]></representative_prompts>
